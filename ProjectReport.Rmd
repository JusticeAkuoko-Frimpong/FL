...
The second function (“FL_combine”) is designed to be used at the central server and combines all the summary statistics from the local servers as described earlier in the report.  The only inputs necessary are the direct outputs from the first function.
The third function (“FL”) runs the federated learning analysis to produce the results of the study.  The input for this function is the output from the second function.
Having these functions integrated into each other allows for our analysis to be run in a seamless way with reduced risk of error.
...
Our project treated different insurance companies as the central server and used the equation [***].  For all 5 local servers we ran the “FL_local_sumamry” function and sent only the summary statistics to the central server.  We then ran the “FL_combine” function.  Finally, we ran the “FL” function and got the results below.
